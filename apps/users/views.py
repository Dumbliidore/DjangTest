        ...
        if login_form.is_valid():
            ...
            if user is not None:
                if user.is_active:
                    ...
                    email = UserProfile.objects.get(username=user_name).email
                    send_register_email(email, send_type='register')
                    return render(request, 'login.html', {'login_form': login_form, "msg": "用户未激活！"})
            else:
                return render(request, 'login.html', {'login_form': login_form, "msg": "用户或密码错误！"})
        else:
...
class RegisterView(View):
    ...
    def post(self, request):
        ...
        if register_form.is_valid():
            ...
            if UserProfile.objects.filter(username=user_name) and UserProfile.objects.filter(email=email):
                return render(request, 'register.html', {'register_form': register_form, 'msg': '该用户名与邮箱已被注册'})
            if UserProfile.objects.filter(email=email):
                return render(request, 'register.html', {'register_form': register_form, 'msg': '该邮箱已被注册'})
            if UserProfile.objects.filter(username=user_name):
                return render(request, 'register.html', {'register_form': register_form, 'msg': '该用户名已被注册'})
            pass_word = request.POST.get('password', '')
            ...
            send_register_email(email, send_type='register')
            # send_register_email(email='1227314815@qq.com', send_type='register')
            return render(request, 'login.html')
        else:
            return render(request, 'register.html', {'register_form': register_form})
...
class UserInfoView(LoginRequiredMixin, View):
    ...
    def get(self, request):
        sort = 'info'
        return render(request, 'usercenter-info.html', {'sort': sort})
